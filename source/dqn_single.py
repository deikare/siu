...
class DqnSingle():
    ...
    def __init__(self, env: TurtlesimEnvBase, id_prefix='dqns', seed=42):
        ...
        self.DISCOUNT = .8  # D dyskonto dla nagrody w następnym kroku
        self.EPS_INIT = 1.0  # µ początkowy
        self.EPS_DECAY = .99  # E spadek µ
        self.EPS_MIN = .05  # e µ minimalny
        ...
        self.EPISODES_MAX = 2000  # P liczba epizodów uczących
        ...
        self.SAVE_MODEL_EVERY = 200  # co ile epizodów zapisywać model
        random.seed(seed)
        np.random.seed(seed)
        self.model = None
    ...
    # sygnatura eksperymentu, tj. wartości parametrów w jednym łańcuchu znaków - używane do nazywania plików z wynikami
    ...
    def decision(self, the_model, last, cur):
        ...
        return the_model.predict(np.expand_dims(inp, axis=0)).flatten()  # wektor przewidywanych nagród dla sterowań
    ...
    def make_model(self):
        ...
        # optymalizator - niektórzy używają odl. Hubera i learning_rate +- rząd wielkości
    ...
    def train_main(self, tname: str, save_model=True, save_state=True):
        ...
        for episode in range(self.EPISODES_MAX):  # ucz w epizodach treningowych
            ...
            print(
                f' {np.nanmean(episode_rewards[episode - 19:episode + 1] / 20.0 / env.MAX_STEPS):.3f}')  # śr. nagroda za krok
            if (episode + 1) % self.SAVE_MODEL_EVERY == 0 and save_model:
                self.model.save(f'models/{dqns.xid()}.h5')  # zapisz bieżący model na dysku
            if (episode + 1) % self.SAVE_MODEL_EVERY == 0 and save_state:  # zapisz bieżący stan uczenia
                pickle.dump((episode, episode_rewards, epsilon, self.replay_memory),
                            open(f'models/{dqns.xid()}.pkl', 'wb'))
    ...
    def do_train(self):
        ...
        self.model.fit(np.stack(X), np.stack(y), batch_size=self.TRAINING_BATCH_SIZE, verbose=0, shuffle=False)


# przykładowe wywołanie uczenia
if __name__ == "__main__":
    env = turtlesim_env_single.provide_env()  # utworzenie środowiska
    env.setup('scenariusz3.csv', agent_cnt=1)  # połączenie z symulatorem
    env.PI_BY = 6  # zmiana wybranych parametrów środowiska
    env.DIST_RWRD_RATE = 3.0
    env.SPEED_RWRD_RATE = 5.0
    env.MAX_STEPS = 10
    env.CAM_RES = 100
    agents = env.reset()  # ustawienie agenta
    tname = list(agents.keys())[0]
    dqns = DqnSingle(env, id_prefix='mk1-c16c16c16d128-M-lr001')  # utworzenie klasy uczącej
    dqns.make_model()  # skonstruowanie sieci neuronowej
    dqns.train_main(tname, save_model=True, save_state=True)  # wywołanie uczenia (wyniki zapisywane okresowo)
