...
class TurtleAgent:  # struktura ze stałymi i bieżącymi atrybutami agenta
    pass  # route, sec_id, seq, color_api, goal_loc, pose, map, fd  - przypisywane na bieżąco
...
class TurtlesimEnvBase(metaclass=abc.ABCMeta):
    ...
    def __init__(self):
        ...
        self.CAM_RES = 200              # dB. boku siatki [px]
        self.SEC_PER_STEP = 1.0         # okres dyskretyzacji sterowania - nie mniej ni| 1 [s]
        self.WAIT_AFTER_MOVE = .01      # oczekiwanie po setPose() [s]
        # parametry oceny sytuacyjnej
        self.SPEED_RWRD_RATE = 2.0      # wzmocnienie nagrody za jazdę w kierunku
        ...
        self.agents = None              # sBownik stanów agentów {tname:TurtleAgent}
    ...
    def setup(self,
        ...
        self.routes = {}
        self.agents = {}
        # załadowanie tras agentów do self.routes
        with open(routes_fname, "r") as f:
            reader = csv.reader(f, delimiter=";")
            for line in reader:
                section = (int(line[1]), float(line[2]), float(line[3]), float(line[4]), float(line[5]), float(line[6]),
                           float(line[7]))
                if (line[0] not in self.routes):
                    self.routes[line[0]] = []
                self.routes[line[0]].append(section)
        ...
        for route, sections in self.routes.items():  # dla kolejnych tras
            for sec_id, sec in enumerate(sections):  # dla kolejnych odcinków trasy
                for seq in range(sec[0]):  # utwórz określoną liczbę żółwi
                    ...
                    p = Pose(x=10, y=10, theta=0)  # obrót żółwia
                    self.tapi.spawnTurtle(tname, p)
    ...
    def reset(self,
        ...
        for tidx, tname in enumerate(tnames):
            ...
            if sections[tidx] == 'default':  # żółw pozycjonowany wg csv
                section = self.routes[agent.route][agent.sec_id]
            elif sections[tidx] == 'random':  # żółw pozycjonowany w losowym segmencie trasy
                section_cnt = len(self.routes[agent.route])
                sec_id = random.randrange(0, section_cnt)
                section = self.routes[agent.route][sec_id]
            else:  # żółw pozycjonowany we wskazanym segmencie (liczone od 0)
                section = self.routes[agent.route][sections[tidx]]
            agent.goal_loc = Pose(x=section[5] / self.px_meter_ratio, y=section[6] / self.px_meter_ratio)  # pierwszy cel
            # próba ulokowania agenta we wskazanym obszarze i jednocześnie na drodze (niezerowy wektor zalecanej prędkości)
            while True:
                x = np.random.uniform(section[1], section[2]) / self.px_meter_ratio
                y = np.random.uniform(section[3], section[4]) / self.px_meter_ratio
    ...
    def get_map(self, tname: str):
        ...
        # pobranie rastra sytuacji żółwia - każda komórka zawiera uśr. kolor planszy, odl. od celu i flagę obecności
        # innych agentów
