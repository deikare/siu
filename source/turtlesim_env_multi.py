...
class TurtlesimEnvMulti(TurtlesimEnvBase):
    ...
    def step(self,actions,realtime=False):              # {id_żółwia:(prędkość,skręt)}
        ...
        if realtime:                                    # jazda+skręt+jazda
            for tname,action in actions.items(): # to samo, co wcześniej, tylko że dla każdego agenta (zdefiniowanego przez listę akcji)
                twist = Twist()
...
if __name__ == "__main__":
    ...
    time.sleep(1)
    agents = env.reset(sections="random") #można też puścić listę tagents w postaci nazw agentów
    ...
    for step in range(10):
        ...
        env.step(actions=actions, realtime=True)
