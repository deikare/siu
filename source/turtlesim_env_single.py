...
class TurtlesimEnvSingle(TurtlesimEnvBase):
    ...
    def step(self, actions, realtime=False):
        # TODO self.max_steps ustawiać niezależnie od reset()
        ...
        # przejechać 1/2 okresu, skręcić, przejechać pozostałą 1/2
        if realtime:  # jazda+skręt+jazda
            twist = Twist()
            twist.linear.x = action[0]
            twist.linear.y = 0
            twist.angular.z = action[1]
            half_time = 0.5 * self.SEC_PER_STEP
        ...
        self.agents[tname].pose = pose1  # MK: nowy wiersz
        ...
        if fv1 > .001:
            ...
            if vf1 > 0:
                ...
                r2 = -self.SPEED_RVRS_RATE * vf1  # kara za jazdę pod prąd MK: zmiana
        ...
        print(f'RWD: {reward:.2f} = {fa1:.2f} * ({r1:.2f}speed {r2:.2f}flow) {r3:.2f}goal {r4:.2f}out')
...
# TODO (*) 10 kroków 1 żółwia z losowego segmentu, z losową prędkością <0,2;1> i skrętem <-0,3;0,3>
if __name__ == "__main__":
    import random

    env = provide_env()
    env.setup("scenariusz1.csv", agent_cnt=1)
    env.reset()
    for i in range(0, 10):
        velocity = random.uniform(0.2, 1)
        turn = random.uniform(-0.3, 0.3)
        actions = {0: {0: velocity, 1: turn}}
