...
class TurtlesimEnvSingle(TurtlesimEnvBase):
    def __init__(self):
        super().__init__()
    def step(self,actions,realtime=False):                                                                                  # TODO self.max_steps ustawiać niezależnie od reset()
        ...
        # TODO przejechać 1/2 okresu, skręcić, przejechać pozostałą 1/2
        if realtime:                                    # jazda+skręt+jazda
            ...
            for twistNumber in range(2):
                self.tapi.setVel(tname, twist)
                rospy.sleep(0.5 * self.SEC_PER_STEP)
        else:                                           # skok+obrót
...
if __name__ == "__main__":
    ...
    for step in range(10):
        action = [random.uniform(0.2, 1.0), random.uniform(-0.3, 0.3)]
